BINARY_NAME := otovinn-auth
HTTP_MAIN_PACKAGE_PATH := cmd/http
PROTO_DIR := internal/adapter/transport/grpc/proto
PB_DIR := internal/adapter/transport/grpc/proto/pb
PROTOC := protoc
GRPC_PLUGIN := protoc-gen-go
GRPC_GATEWAY_PLUGIN := protoc-gen-grpc-auth
...
run-http:
    ...
    CGO_ENABLED=0 go run auth/$(HTTP_MAIN_PACKAGE_PATH)
